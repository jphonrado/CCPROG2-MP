...
int getUserIndexNoPassword(char username[MAX_CHAR_USER], UserInfo newUser[MAX_USERS], int numUsers);
int isUserValid(char username[MAX_CHAR_USER], char password[MAX_CHAR_PASS], UserInfo newUser[MAX_USERS], int numUsers);
int getUserIndex(char username[MAX_CHAR_USER], char password[MAX_CHAR_PASS], UserInfo newUser[MAX_USERS], int numUsers);
...
void LoginPage(char adminPass[MAX_CHAR_PASS], UserInfo newUser[MAX_USERS], int *numUsers, char resetRequests[MAX_CHAR_USER][MAX_USERS], int *numResetRequests, 
        ...
        messageTag sentMessages[MAX_MESSAGES], messageTag Announcements[MAX_MESSAGES], messageTag Received[MAX_MESSAGES]);
void modifyAccountSecurity(UserInfo newUser[MAX_USERS], int numUsers, int userIndex);
...
void userModulePage(UserInfo newUser[MAX_USERS], int numUsers, char *username, int userIndex, 
    int *sentCount, int *announcementCount, int *receiveCount, int *numMessages, messageTag messages[MAX_MESSAGES], 
    messageTag sentMessages[MAX_MESSAGES], messageTag Announcements[MAX_MESSAGES], messageTag Received[MAX_MESSAGES]);
void modifyPersonalContents(UserInfo newUser[MAX_USERS], int numUsers, char *username, int userIndex);
void modifyPersonalConnectionsPage(UserInfo newUser[MAX_USERS], int numUsers, int userIndex);
void addPersonalConnection(UserInfo newUser[MAX_USERS], int numUsers, int userIndex);
void removePersonalConnection(UserInfo newUser[MAX_USERS], int numUsers, int userIndex);
...
void browseUsers(UserInfo newUser[MAX_USERS], int numUsers, int userIndex, char *username, messageTag SavedMessages[MAX_MESSAGES], int *savedCount);
void viewAllUsers(UserInfo newUser[MAX_USERS], int numUsers, int userIndex, char *username, messageTag SavedMessages[MAX_MESSAGES], int *savedCount);
void filterUsersByName(UserInfo newUser[MAX_USERS], int numUsers, int userIndex, char *username, messageTag SavedMessages[MAX_MESSAGES], int *savedCount);
void viewUserPage(UserInfo newUser[MAX_USERS], int numUsers, int targetIndex, int userIndex, char *username, messageTag SavedMessages[MAX_MESSAGES], int *savedCount);
...
void addUserConnection(UserInfo newUser[MAX_USERS], int numUsers, int userIndex, int targetIndex);
    ...
    if (pFile != NULL) {
        ...
        *numMessages = atoi(strTemp);
        ...
        while (i < *numMessages) {
            ...
            Messages[i].strMessageEntry = malloc(sizeof(char *) * (size_t)numLines);
            j = 0;
            while (j < numLines) {
                fgets(strTemp, sizeof(strTemp), pFile);
                clean(strTemp);
...
void cipherPassword(char *password, char *cipheredPassword) {
    ...
    for (i = 0; password[i] != '\0'; i++) {
        cipheredPassword[i] = (char)(password[i] + shift);
    }
    ...
    while (!bQuit) {
        ...
        switch (nChoice) {
            case 1:
                ...
                break;
            case 2:
                AdminHandleMessagesModulePage(newUser, numUsers, numMessages, messages);
                break;
            case 3:
...
void AdminModuleLogin(char adminPass[MAX_CHAR_PASS], UserInfo newUser[MAX_USERS], int numUsers, char resetRequests[MAX_CHAR_USER][MAX_USERS], int *numResetRequests, int *numMessages, messageTag messages[MAX_MESSAGES]) {
    ...
    while (attempts > 0 && !bGoBack) {
        ...
        if (strcmp(enteredPass, "back") == 0) {
            ...
            AdminModulePage(adminPass, newUser, numUsers, resetRequests, numResetRequests, numMessages, messages);
			system("pause");
        } 
		else {
            attempts--;
